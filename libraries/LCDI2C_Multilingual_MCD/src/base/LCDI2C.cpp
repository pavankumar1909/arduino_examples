...
inline void LCDI2C::write(uint8_t value) { send(value, Rs); }

#endif
#include "Wire.h"

static bool wireBegan =
    false; //// Added by Loc P. Le not to call Wire.begin() multiple times
...
inline void LCDI2C::command(uint8_t value) { send(value, 0); }
...
void LCDI2C::cursor_on() { cursor(); }

void LCDI2C::cursor_off() { noCursor(); }

void LCDI2C::blink_on() { blink(); }

void LCDI2C::blink_off() { noBlink(); }
...
void LCDI2C::draw_horizontal_graph(uint8_t row, uint8_t column, uint8_t len,
                                   uint8_t pixel_col_end) {}
void LCDI2C::draw_vertical_graph(uint8_t row, uint8_t column, uint8_t len,
