/*
  ** LCDI2C_UTF8 class implementation **
  Base class implementation for printing UTF8 strings (NFC normalized) to I2C
  LCD.
...
void LCDI2C_UTF8::setAutoNewLine(bool stat = true) { autoNewLine = stat; }
...
uint16_t LCDI2C_UTF8::getCodePoint(const byte text[], uint8_t &nbytes) {
  ...
  nbytes = 1;
  if (text[0] < 0x80)
    return (uint16_t)text[0];
  else if (text[0] < 0xC2)
    return result;   // Invalid UTF8
  uint16_t units[4]; // Decomposed byte codes, each unit size large enough for
                     // whole code point
  byte byte0 = text[0];
  ...
  for (int i = 0; i < 4; i++) { // Max 4 bytes (presented by 4 first bits)
    ...
    if (byte0 & 0x80) {         // Left shift while next bit = 1
      nbytes++;
      units[i + 1] =
          text[i + 1] & 0x7F; // Extract approriate component of the code point
    } else {
      units[0] =
          byte0 >> nbytes; // Right shift to start position without first bits 1
...
uint8_t LCDI2C_UTF8::nextWordLength(const byte text[], uint16_t start,
                                    uint16_t code) {
  ...
  while (text[i] != '\0') { // Find first non-letter character from start
    if ((text[i] < 0x30) || // Below '0'
        ...
        (text[i] > 0x7A &&
         text[i] < 0x80)) { // Between 'z' and first multi-byte UTF8 character
      return length;
...
size_t LCDI2C_UTF8::print(const char text[], int pause) {
  ...
  for (uint16_t i = 0; text[i] != '\0'; i += nbytes, nletters++) {
    // Convert to appropriate code point (UTF-16). Out: nbytes = bytes used in
    // the convetion.
    ...
    if (autoNewLine && (cursorColumn > nextWordEndCol)) {
      length = nextWordLength(
          (const byte *)text, i,
          code); // Note that we don't check again on the same word
      if (length)
        nextWordEndCol =
            (length > colNum ? colNum - 1 : cursorColumn + length - 1);
      if (nextWordEndCol >=
          colNum) { // Not enough columns to print the whole word?
        if ((cursorRow + 1) % rowNum) { // Print it if next row exists
          newLine(); // Automatically move to the next row to print the whole
                     // next word
          if (code == 0x20)
            skip_print = true; // Don't print leading space of next word
        } else {               // No row left to print
          if (pause > 0)
            delay(pause * 1000);
          clear(); // Automatically clear LCD and CGRAM for printing the whole
                   // next word
        }
      }
    }
    ...
    if (!skip_print &&
        cursorColumn < colNum) { // The leading space not printed after auto
                                 // moving, and don't move the cursor
      ...
      if (ord < 0x100) // Only one character returned
        ...
        ++cursorColumn; // Move 1 pos cause printing 2 characters at once
                        // (having enough space)
      }

      // Move cursor to next position
      if (++cursorColumn ==
          colNum) { // Check if the cursor exceeds max column of LCD screen?
        if (text[i + nbytes] != '\0') { // Check if the end of 'text'?
          if (autoNewLine) {
            if ((cursorRow + 1) %
                rowNum) { // Check if existing rows left to print?
              newLine();  // Automatically move to the next row if the cursor
                          // exceeds colNum
            } else {
              if (pause >= 0)
                delay(pause * 1000);
              clear(); // Automatically clear LCD and CGRAM if no space left to
                       // print
            }
          } else { // autoNewLine
            cursorColumn =
                colNum; // Keep the cursor staying at the end of LCD screen
          }
        }
      }
    }
...
size_t LCDI2C_UTF8::print(int n, int base) { return print((long)n, base); }
...
size_t LCDI2C_UTF8::println(int n, int base) { return println((long)n, base); }
